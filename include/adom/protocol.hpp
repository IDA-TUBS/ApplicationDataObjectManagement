...
struct EntityDescriptor
{
    ...
    EntityDescriptor(uint16_t g_entity_id, const char g_ip_address[string_length], uint16_t g_ingress_port) 
        : entity_id(g_entity_id), ingress_port(g_ingress_port) {
        std::memcpy(ip_address, g_ip_address, sizeof(ip_address)); // Use memcpy to copy string.
    }
...
struct DataObjectInstance{
    ...
    DataObjectInstance(Topic topic, uint16_t updated_sequence_nr) : 
        object_type(topic), object_sequence_nr(updated_sequence_nr) {}
};
...
struct DataBlockHeader{
    ...
    uint16_t block_id;
    uint16_t block_size;
    uint64_t data_address_offset;   // Is always 0 for unstructured ObjectSample that is not one dimensional

    DataBlockHeader() 
        : associated_object(), block_id(0), block_size(0), data_address_offset(0) {}


    DataBlockHeader(Topic topic, uint16_t updated_sequence_nr, uint16_t block_id, uint16_t block_size, uint64_t data_address_offset):
        associated_object(),
        block_id(block_id),
...
public:
    uint64_t associated_request_id_;
