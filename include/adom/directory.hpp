...
#include <adom/guid/guidPrefixManager.hpp>
#endif
...
class BaseObjectSample {
    public:
        virtual ~BaseObjectSample() = default;
    
        virtual EntityDescriptor getHomeDirectory() const = 0;
        virtual Topic getTopic() const = 0;
        virtual uint16_t getSequenceNumber() const = 0;
        virtual uint16_t getBlockCount() const = 0;
        virtual Structure getStructure() const = 0;
        virtual std::vector<unsigned char>* getDataPointer() = 0;

        virtual DataBlockHeader getHeader(uint16_t block_id) const = 0;

        virtual void writeBlock( const DataBlockHeader& hdr,
            const unsigned char* data,
            size_t len ) = 0;

        virtual unsigned char* readBlock(uint16_t block_id) = 0;

        virtual void translateData(unsigned char* output_data) = 0;
        
        virtual void printObjectSample() const = 0;
        
        // Block-Handling
        virtual int isAvailable(int block_id) const = 0;
        virtual void makeAvailable(int block_id) = 0;
};
 ...
 * 
 */
class ObjectSample: public BaseObjectSample  {
    public:
    ...
    ObjectSample(EntityDescriptor home_directory, Topic topic, uint16_t updated_sequence_nr, unsigned char *data, Structure structure){
        ...
        size_t total_size = structure.object_width
                      * structure.object_height
                      * structure.object_channels;
         
        object_sample_data_.resize(total_size);      

        std::memcpy(object_sample_data_.data(), data, total_size);
    ...
    EntityDescriptor getHomeDirectory() const override {
        return home_directory_descriptor_;
    };

    Topic getTopic() const override {
        return topic_;
    };
    ...
    uint16_t getBlockCount() const override {
        return block_count_;
    };
    ...
    std::vector<unsigned char>* getDataPointer() override {
        return &object_sample_data_;
    };
    
    DataBlockHeader getHeader(uint16_t block_id) const override {
        uint64_t address_offset = 0;  // Is always 0 for unstructured ObjectSamples that are not one dimensional
        DataBlockHeader header {topic_, sequence_nr_, block_id, block_size_, address_offset};
        return header;
    };

    int isAvailable(int block_id) const override {
    ...
    void makeAvailable(int block_id) override {
        if (block_id > block_count_) {
            throw std::runtime_error("Invalid Block ID!");
            return;
        }
    ...
    void printObjectSample() const override {
    ...
    void writeBlock(const DataBlockHeader& hdr, const unsigned char* data, size_t len) override {
        if (hdr.block_id >= block_count_) {
            throw std::out_of_range("Invalid block_id in readBlock");
        }

        if (hdr.block_size != block_size_){
            throw std::runtime_error("Data structure missmatch.");
        }

        if (len < block_size_)
        throw std::runtime_error("Data-pointer too short");

        int block_height = data_structure_.object_height / data_structure_.block_rows;
        int block_width = data_structure_.object_width / data_structure_.block_cols;
        int block_index = hdr.block_id;

        
        if (data_structure_.type == ONE_DIMENSIONAL){
            memcpy(&object_sample_data_[block_index * hdr.block_size], &data[block_index * hdr.block_size], hdr.block_size);
            throw std::runtime_error("Currently not implemented for One-dimensional data.");

        } else if(data_structure_.type == TWO_DIMENSIONAL) {
            if (data_structure_.object_width % data_structure_.block_cols != 0 || data_structure_.object_height % data_structure_.block_rows != 0) {
                throw std::runtime_error("Invalid image dimensions. Width must be divisible by block_cols and height by block_rows.");
            }

            int row = block_index / data_structure_.block_cols;
            int col = block_index % data_structure_.block_cols;
            int start_row = row * block_height;
            int start_col = col * block_width;

            size_t row_bytes = static_cast<size_t>(block_width) * data_structure_.object_channels;

            for (int r = 0; r < block_height; ++r) {
                size_t src_off  = static_cast<size_t>(r) * row_bytes;
                size_t dest_off = (static_cast<size_t>(start_row + r) * data_structure_.object_width + start_col) * data_structure_.object_channels;

                if (src_off + row_bytes > block_size_)
                    throw std::out_of_range("read src beyond block_size");
                if (dest_off + row_bytes > object_sample_data_.size())
                    throw std::out_of_range("write dest beyond object_sample_data");
    
                std::memcpy(object_sample_data_.data() + dest_off, data + src_off, row_bytes);    
            }
        } 

        makeAvailable(block_index);
    };
    
    unsigned char* readBlock(uint16_t block_id) override {
        unsigned char* buf = new unsigned char[block_size_];

        if (data_structure_.type == ONE_DIMENSIONAL) {
            size_t offset = block_id * block_size_;
            std::memcpy(buf,
                        object_sample_data_.data() + offset,
                        block_size_);
    
        } else if (data_structure_.type == TWO_DIMENSIONAL) {
            int cols = data_structure_.block_cols;
            int rows = data_structure_.block_rows;
            int bw   = data_structure_.object_width  / cols;
            int bh   = data_structure_.object_height / rows;
            int ch   = data_structure_.object_channels;
            ...
            size_t row_bytes = bw * ch;
            size_t dst_off   = 0;
            
            for (int y = 0; y < bh; ++y) {
                size_t src_off = ((start_row + y) * data_structure_.object_width
                                  + start_col)
                                 * ch;

                
                // Bound‐Checks
                size_t total = object_sample_data_.size();
                if (src_off + row_bytes > total) {
                throw std::out_of_range(
                    "readBlock: source offset + len exceeds buffer: "
                    + std::to_string(src_off) + "+" + std::to_string(row_bytes)
                    + " > " + std::to_string(total));
                }

                if (dst_off + row_bytes > block_size_) {
                throw std::out_of_range(
                    "readBlock: dest offset + len exceeds block_size: "
                    + std::to_string(dst_off) + "+" + std::to_string(row_bytes)
                    + " > " + std::to_string(block_size_));
                }

                std::memcpy(buf + dst_off,
                            object_sample_data_.data() + src_off,
                            row_bytes);
                dst_off += row_bytes;
            }
        }
    ...
    void translateData(unsigned char* output_data) override {
        size_t object_data_size = data_structure_.object_width * data_structure_.object_height * data_structure_.object_channels;
        memcpy(output_data, object_sample_data_.data(), object_data_size);
        return;
    }
    ...
    std::vector<int> availability_matrix_;  // The matrix depends on the structure; on Publisher, this matrix is technically not necessary.
 ...
 * 
 */
class StructuredObjectSample : public BaseObjectSample  {
    public:
    

    StructuredObjectSample(EntityDescriptor home_directory, Topic topic, uint16_t updated_sequence_nr, unsigned char *data, Structure structure){
        home_directory_descriptor_ = home_directory;
        topic_ = topic;
        sequence_nr_ = updated_sequence_nr;
        data_structure_ = structure;
        ...
        for (uint16_t i = 0; i < block_count_; i++){
            ...
            uint64_t address_offset = &object_sample_data_[i*block_size_] - &object_sample_data_[0];
    ...
    void writeBlock(const DataBlockHeader& hdr, const unsigned char* data, size_t len) override {
        memcpy(object_sample_data_.data() + hdr.data_address_offset, data, len);
        
        makeAvailable(hdr.block_id);
    };
    
    unsigned char* readBlock(uint16_t block_id) override {
        return object_sample_data_.data() + getHeader(block_id).data_address_offset;
    };

    void translateData(unsigned char* output_data) override {
        translate_to_uchar(*this->getDataPointer(), output_data, this->getStructure());
        return;
    }
    ...
    DataBlockHeader getHeader(uint16_t block_id) const override {
        if (block_id >= static_cast<uint16_t>(block_header_list_.size())) {
            throw std::runtime_error("Invalid Block ID!");
        }
        return block_header_list_[block_id];
    };

    std::vector<DataBlockHeader> getHeader() const {
...
class Directory {
    public:
            ...
            socket_endpoint_ingr_(boost::asio::ip::address::from_string(directory_descriptor_.ip_address), directory_descriptor_.ingress_port)
        {
                ...
                adom_socket_ingr_.non_blocking(true);
         ...
         * @param entity_descriptor 
         */
        Directory(EntityDescriptor entity_descriptor, EntityDescriptor other_entity_descriptor, SafeQueue<std::pair<DataRequest, udp::endpoint>> &directory_request_queue, SafeQueue<DataTransport> &directory_reply_queue, SafeQueue<std::pair<DataAnnouncement, udp::endpoint>> &directory_announcement_queue):
            directory_descriptor_(entity_descriptor), 
            other_directory_descriptor_(other_entity_descriptor),
            directory_request_queue_(directory_request_queue),
            directory_reply_queue_(directory_reply_queue),
            directory_announcement_queue_(directory_announcement_queue),
            recvMsgs{},
         ...
         * @param entity_descriptor 
         */
        Directory(uint32_t host_id, EntityDescriptor entity_descriptor, SafeQueue<std::pair<DataRequest, udp::endpoint>> &directory_request_queue, SafeQueue<DataTransport> &directory_reply_queue, SafeQueue<std::pair<DataAnnouncement, udp::endpoint>> &directory_announcement_queue):
            host_id(host_id),
            directory_descriptor_(entity_descriptor), 
            directory_request_queue_(directory_request_queue),
            directory_reply_queue_(directory_reply_queue),
         ...
         */
        void addNewObjectSample(std::shared_ptr<BaseObjectSample> new_sample);
         ...
         */
        std::vector<uint16_t> checkAvailability(std::shared_ptr<BaseObjectSample> sample, std::vector<uint16_t> block_ids);
         ...
         * @return                      Request ID
         */
        int sendDataRequest(EntityDescriptor home_directory, std::shared_ptr<BaseObjectSample>, std::vector<uint16_t> block_ids);
         ...
         */
        std::shared_ptr<BaseObjectSample> getLastObjectSample(Topic type);
         ...
         * @return std::shared_ptr<ObjectSample>    shared pointer to most up-to-date sample
         */
        std::shared_ptr<BaseObjectSample> getSample(Topic type, int sequence_nr);
         ...
         */
        void setRequestedBlocks(uint64_t request_id, const std::set<uint16_t>& requested_blocks);
    ...
    private:
         ...
         */
        void distributeAnnouncements(std::shared_ptr<BaseObjectSample> new_sample);
         ...
         * @param id_tuple          Tuple of ID of the request and ID of requester
         */
        void startDataTransport(std::list<DataBlockHeader> header_list, udp::endpoint udp_endpoint, std::pair<uint64_t, uint16_t> id_tuple);
         ...
         *          All messages, whether DataRequests, DataTransports or DataAnnouncements are are sent in a wrapper called ADOM message.
         * 
         * @param msg       message that contains DataRequests, DataTransports or DataAnnouncements
         * @param target    specified target destination of the message
         */
        void send_msg(scps::MessageNet_t& msg, udp::endpoint target);
#endif
        ...
        #if(RTPS)
        scps::GUID_t guid;   // rtps guid
        uint32_t host_id;        
        scps::NetMessageParser *netParser; // (sub)message parser and encoder
        #endif
         ...
         *                                        
         */
        std::map<std::pair<Topic, uint16_t>, std::shared_ptr<BaseObjectSample>> tracked_object_samples_;
        ...
        std::shared_mutex data_access_lock_;
