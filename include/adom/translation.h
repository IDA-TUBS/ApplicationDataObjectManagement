...
struct Structure{
    ...
    unsigned char * serialize() {
        ...
        std::memcpy(buffer + offset, &block_rows, sizeof(block_rows));
        offset += sizeof(block_rows);
    
        std::memcpy(buffer + offset, &block_cols, sizeof(block_cols));
        offset += sizeof(block_cols);
    
        std::memcpy(buffer + offset, &object_height, sizeof(object_height));
        offset += sizeof(object_height);
    
        std::memcpy(buffer + offset, &object_width, sizeof(object_width));
        offset += sizeof(object_width);
    
        std::memcpy(buffer + offset, &object_channels, sizeof(object_channels));
    
        return buffer;
    };
    
    static Structure deserializeStructure(const unsigned char* buffer, const size_t bufferSize) {
        size_t expectedSize = sizeof(StructureType)
                            + sizeof(uint16_t) * 5;
        if(bufferSize < expectedSize) {
            throw std::runtime_error("Buffer too small for Structure Deserialization.");
        }
    
        Structure s;
        size_t offset = 0;
    
        std::memcpy(&s.type, buffer + offset, sizeof(s.type));
        offset += sizeof(s.type);
    
        std::memcpy(&s.block_rows, buffer + offset, sizeof(s.block_rows));
        offset += sizeof(s.block_rows);
    
        std::memcpy(&s.block_cols, buffer + offset, sizeof(s.block_cols));
        offset += sizeof(s.block_cols);
    
        std::memcpy(&s.object_height, buffer + offset, sizeof(s.object_height));
        offset += sizeof(s.object_height);
    
        std::memcpy(&s.object_width, buffer + offset, sizeof(s.object_width));
        offset += sizeof(s.object_width);
    
        std::memcpy(&s.object_channels, buffer + offset, sizeof(s.object_channels));
