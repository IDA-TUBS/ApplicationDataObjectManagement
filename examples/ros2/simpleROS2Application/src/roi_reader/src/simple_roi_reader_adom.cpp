...
public:
      ...
      : Node("simple_roi_reader_adom")
  {
    ...
    adom_interface_ = std::make_unique<ApplicationInterface>("DIRECTORY_02", dir_cfg_path, topic_cfg_path, setup_path);
    adom_interface_->initialize();

    // register IMAGE topic, so that the directory starts managing associated data samples
    adom_interface_->registerNewTopic("IMAGE");
    ...
    sync_->registerCallback(std::bind(&RoiReader::callback, this, std::placeholders::_1, std::placeholders::_2));
