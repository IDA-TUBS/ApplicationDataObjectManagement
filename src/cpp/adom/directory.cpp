...
void Directory::initiate(){
    ...
    scps::GuidPrefix_t guidPrefix;
    scps::guidPrefixManager::instance().create(
        uint32_t(0x512),
        host_id, 
        guidPrefix
    );
    ...
    // set guid
    guid = scps::GUID_t(guidPrefix, directory_descriptor_.entity_id);
    #endif
 ...
 */
void Directory::addNewObjectSample(std::shared_ptr<BaseObjectSample> new_sample){
    {  
        std::unique_lock lock(data_access_lock_); // Mutex-Protection
 ...
 */
void Directory::distributeAnnouncements(std::shared_ptr<BaseObjectSample> new_sample){
    /**
     * @todo
    ...
    if (!connected_directories_.empty()){

        for(std::list<EntityDescriptor>::iterator it = connected_directories_.begin(); it != connected_directories_.end(); ++it){

#if(RTPS)

            // create SCPS header
            scps::SCPSHeader *header = new scps::SCPSHeader(guid.prefix);

            scps::DataAnnouncementMsg *announcement = new scps::DataAnnouncementMsg(it->entity_id, 
                                                                                    directory_descriptor_.entity_id, 
                                                                                    new_sample->getTopic(),
                                                                                    new_sample->getSequenceNumber(), 
                                                                                    sizeof(new_sample->getStructure()),
                                                                                    new_sample->getStructure().serialize(),
                                                                                    std::chrono::system_clock::now()

            );

            scps::MessageNet_t *txMsg = new scps::MessageNet_t;
            header->headerToNet(txMsg);
            announcement->dataToNet(txMsg); 
            
            logDebug("Sending announcement of sample " + std::to_string(new_sample->getSequenceNumber()) + " to directory " + std::to_string(it->entity_id) + "...");

            // create endpoint from home_directory
            udp::endpoint other_dir_endpoint(boost::asio::ip::address::from_string(it->ip_address), it->ingress_port); 

            send_msg(*txMsg, other_dir_endpoint);
        ...
        logDebug("List of connected directories is empty...");
    }
...
void Directory::writeObjectBlock(DataTransport * data_block_message){
...

    {
        ...
        if (it == tracked_object_samples_.end()) {
            logError("No associated sample found for type: " +
                     std::to_string(block_header.associated_object.object_type) +
                     ", seq: " +
                     std::to_string(block_header.associated_object.object_sequence_nr));
            return;
        }

        auto sample_ptr = it->second; // shared_ptr<BaseObjectSample>
        if (!sample_ptr) {
            logError("Affiliated sample pointer is null.");
            return;
        }

        const unsigned char* payload = reinterpret_cast<const unsigned char*>( data_block_message->data_.data() );

        sample_ptr->writeBlock(block_header, payload, data_block_message->block_size_);
...
void Directory::receiveMessages(void){
    ...
    {      
        while(directory_running_.load() && ret_val == 0){
            ...
            ret_val = adom_socket_ingr_.receive_from(boost::asio::buffer(message.buffer, message.max_size), sender_endpoint, 0, error);
            message.length = ret_val;
        }
        ...
        scps::SCPSHeader header;
        header.netToHeader(&message);

        // first extract submessages from msg
        std::vector<scps::SubmessageBase*> res;

        netParser->getSubmessages(&message, &res);

        for(auto subMsg : res)
        {   
            switch (subMsg->subMsgHeader->submessageId){
                case scps::DATA_ANNOUNCEMENT:
                {
                    logDebug("Received Data Announcement! \n");

                    scps::DataAnnouncementMsg *announce_msg = (scps::DataAnnouncementMsg*)(subMsg);

                    // create data announcement struct
                    DataAnnouncement announcement = DataAnnouncement(EntityDescriptor(announce_msg->home_dir_ID, sender_endpoint.address().to_string().c_str(), sender_endpoint.port()), 
                                                                    static_cast<Topic>(announce_msg->object_type),
                                                                    announce_msg->object_sequence_nr,
                                                                    Structure::deserializeStructure(reinterpret_cast<const unsigned char*>(announce_msg->serialized_payload), announce_msg->payload_size));

                    // queue the announcement and the home directory endpoint into the announcement queue for further processing
                    current_announcement = {announcement, sender_endpoint};
                    directory_announcement_queue_.enqueue(current_announcement);
                    
                    //clean up
                    announcement.clear(); // here deconstructor?
                    message.clear();
                    ret_val = 0;
                    break;
                }
                case scps::DATA_REQUEST:
                {
                    logDebug("Received Data Request! \n")

                    
                    scps::DataRequestMsg *request_msg = (scps::DataRequestMsg*)(subMsg);

                    DataRequest request = DataRequest(EntityDescriptor(request_msg->home_dir_ID, sender_endpoint.address().to_string().c_str(), sender_endpoint.port()),
                                                      directory_descriptor_,
                                                      DataObjectInstance(static_cast<Topic>(request_msg->object_type), request_msg->object_sequence_nr),
                                                      request_msg->object_block_count,
                                                      request_msg->object_block_validity,
                    ...
                    //clean up
                    request.clear(); // here deconstructor?
                    message.clear();
                    ret_val = 0;
                    break;
                }

                case scps::DATA_TRANSPORT:
                {                
                    logDebug("Received Data Transport \n");

                    
                    scps::DataTransportMsg *reply_msg = (scps::DataTransportMsg*)(subMsg);

                    DataBlockHeader header = DataBlockHeader(static_cast<Topic>(reply_msg->object_type),
                                                             reply_msg->object_sequence_nr,
                                                             reply_msg->block_ID,
                                                             reply_msg->block_size,
                                                             reply_msg->data_address_offset);

                    // create dataTransport struct
                    DataTransport reply = DataTransport(header,
                                                      reply_msg->serialized_block_payload,
                                                      reply_msg->request_ID);
                    ...
                    //clean up
                    reply.clear(); // here deconstructor?
                    message.clear();
                    ret_val = 0;
                    break;
                }
...
void Directory::DirRequestCallback(DataRequest *request, udp::endpoint target){
    ...
    logDebug("Received Request with ID: " + std::to_string(request->request_id_) + " from directory " + std::to_string(request->reader_application_.entity_id) + " \n");
    logDebug("Collecting relevant headers... \n");

    std::list<DataBlockHeader> header_list = getHeader(request);

    startDataTransport(header_list, target, std::make_pair(request->request_id_, request->reader_application_.entity_id));
}
...
void Directory::DirAnnouncementCallback(DataAnnouncement *dequeued_announcement){
    ...
    // Always use non-strcutured ObjectSample at Subriber/remtoe side to avoid Blockheader overhead for irrelevant data blocks
 ...
 */
std::vector<uint16_t> Directory::checkAvailability(std::shared_ptr<BaseObjectSample> sample, std::vector<uint16_t> block_ids){
 ...
 * @return                      Request ID
 */
int Directory::sendDataRequest(EntityDescriptor home_directory, std::shared_ptr<BaseObjectSample> sample, std::vector<uint16_t> block_ids){
    ...
    size_t validity_bytes = (object_block_count + 7) / 8;
    auto object_block_validity_matrix = new uint8_t[validity_bytes];
    memset(object_block_validity_matrix, 0, validity_bytes);


    for (auto block : block_ids) {
        int matrix_index = block / 8;
        int bit_offset   = block % 8;
        uint8_t mask     = 0b10000000 >> bit_offset;
    ...
    logDebug("Sending Data Request " + std::to_string(request_id) + " for blocks "  + std::to_string(*block_ids.begin()) + " - " + std::to_string(*(--block_ids.end())) + " ...");
    ...
    scps::DataRequestMsg *request = new scps::DataRequestMsg(directory_descriptor_.entity_id,
                                                             home_directory.entity_id,
                                                             associated_object_instance.object_type,
                                                             associated_object_instance.object_sequence_nr,
                                                             request_id,
                                                             object_block_count,
                                                             object_block_validity_matrix,
                                                             sizeof(sample->getStructure()),
                                                             sample->getStructure().serialize(),
                                                             std::chrono::system_clock::now());

    scps::MessageNet_t *txMsg = new scps::MessageNet_t;
    header->headerToNet(txMsg);
    request->dataToNet(txMsg); 
    
    logDebug("Sending announcement of sample " + std::to_string(sample->getSequenceNumber()) + " to directory " + std::to_string(directory_descriptor_.entity_id) + "...");

    // create endpoint from home_directory
    udp::endpoint home_dir_endpoint(boost::asio::ip::address::from_string(home_directory.ip_address), home_directory.ingress_port); 

    send_msg(*txMsg, home_dir_endpoint);
 ...
 */
void Directory::startDataTransport(std::list<DataBlockHeader> header_list, udp::endpoint udp_endpoint, std::pair<uint64_t, uint16_t> id_tuple){

    auto associated_object_frame = header_list.front().associated_object.object_sequence_nr;

    std::shared_lock lock(data_access_lock_); // shared Mutex-Protection for read tracked_object_samples_
    ...
    while (!header_list.empty()) {
        ...
        if (it != tracked_object_samples_.end()) { // Check whether the key exists
            auto associated_sample = it->second; // Access to the shared_ptr
            ...
            if (associated_sample) { // Ensure that the shared_ptr is valid
                ...
                // create data transport message
                scps::DataTransportMsg *data_transport = new scps::DataTransportMsg(id_tuple.second,
                                                                                    directory_descriptor_.entity_id,
                                                                                    associated_sample->getTopic(),
                                                                                    associated_sample->getSequenceNumber(),
                                                                                    id_tuple.first,
                                                                                    header_list.front().block_id,
                                                                                    header_list.front().block_size,
                                                                                    header_list.front().data_address_offset,
                                                                                    associated_sample->readBlock(header_list.front().block_id),
                                                                                    std::chrono::system_clock::now()
                ...
                scps::MessageNet_t *txMsg = new scps::MessageNet_t;
                header->headerToNet(txMsg);
                data_transport->dataToNet(txMsg);            

                send_msg(*txMsg, udp_endpoint);

                #else

                DataTransport block_reply(header_list.front(), associated_sample->readBlock(header_list.front().block_id), id_tuple.first);
                ...
                logDebug( "Sending data block " + std::to_string(header_list.front().block_id) + " for Request "  + std::to_string(id_tuple.first) + "...\n");
...
/**
 * @brief Send message to the specified target.
 *          All messages, whether DataRequests, DataTransports or DataAnnouncements are are sent in a wrapper called ADOM message.
 * 
 * @param msg       message that contains DataRequests, DataTransports or DataAnnouncements
 * @param target    specified target destination of the message
 */
void Directory::send_msg(scps::MessageNet_t& msg, udp::endpoint target){
    std::lock_guard<std::mutex> lock(send_lock_);   

    // send out serialized ADOM message via the directory's dedicated socket
    size_t bytes_sent = adom_socket_ingr_.send_to(boost::asio::buffer(msg.buffer, msg.length), target);
...
DataBlockHeader Directory::getHeader(DataTransport *response){
    ...
    if (it != tracked_object_samples_.end()) {
        auto associated_sample = it->second;
...
std::list<DataBlockHeader> Directory::getHeader(DataRequest *request){
    ...
    for(int i = 0; i < (request->object_block_count_); i++){
 ...
 */
void Directory::setRequestedBlocks(uint64_t request_id, const std::set<uint16_t>& requested_blocks) {
 ...
 */
std::shared_ptr<BaseObjectSample> Directory::getLastObjectSample(Topic type){
 ...
 * @return std::shared_ptr<ObjectSample>    shared pointer to most up-to-date sample
 */
std::shared_ptr<BaseObjectSample> Directory::getSample(Topic type, int sequence_nr){
    std::shared_lock lock(data_access_lock_); // shared Mutex-Protection for read tracked_object_samples_
...
/**
 * @brief This function is called to remove the oldest sample for a specified topic that is managed by the directory. 
